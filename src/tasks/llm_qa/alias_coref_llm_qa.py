...
def run_task(
    ...
    with open(input_jsonl, "r", encoding="utf-8") as fin, open(output_jsonl, "a", encoding="utf-8") as fout:
        for line in fin:
            ...
            alias = str(witness.get("alias", "")).strip()
            evidence = str(witness.get("evidence", "")).strip()
            final_answer = str(item.get("final_answer") or witness.get("canonical") or "").strip()

            if not alias or not evidence or not final_answer:
                n_skip_no_witness += 1
                continue
            ...
            ctx: Dict[str, Any] = {
                ...
                "start_entity": "",
                # skill-level hop count: alias resolution across hops
                "k": 2,
                "alias": alias,
                "evidence": evidence,
            }
            ...
            out_item = {
                ...
                "chain_stats": {
                    "source_k": item_k,
                    "rel_type": witness.get("rel_type"),
                    "selected_hops": witness.get("selected_hops"),
                    "evidence_chunk_ids": witness.get("evidence_chunk_ids"),
