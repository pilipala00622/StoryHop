...
def novelhopqa_constructor(raw_novel_text: str, source_file: str = None) -> List[Dict]:
    ...
    try:
        ...
        if all_final_qa:
            print(f"\n" + "="*50)
            print(f"所有流程完成！生成 {len(all_final_qa)} 个 QA 对")
        else:
...
def read_jsonl_data(file_path: str, max_lines: int = 100) -> List[Dict]:
    """
    从JSONL文件读取数据（限制前N行）
    :param file_path: JSONL文件路径
    :param max_lines: 最大读取行数，默认100行
    :return: 数据列表，每个元素包含source和full_content字段
    """
    data_list = []
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(f, 1):
                ...
                try:
                    ...
                    if "source" not in data or "full_content" not in data:
                        print(f"警告：第{line_num}行缺少必要字段(source或full_content)，跳过")
                        continue
                    data_list.append(data)
                except json.JSONDecodeError as e:
                    print(f"警告：第{line_num}行JSON解析失败：{str(e)}，跳过")
                    continue
        
        print(f"成功读取JSONL文件：{file_path}")
        print(f"限制读取前{max_lines}行，有效数据行数：{len(data_list)}")
        return data_list
    except FileNotFoundError:
        print(f"错误：找不到文件 {file_path}")
        return []
...
# -------------------------- 10. 主函数：支持文件输入 --------------------------
def process_jsonl_file(jsonl_file_path: str, output_file: str, max_lines: int = 100):
    """
    处理JSONL文件，为每一行的full_content生成QA对
    :param jsonl_file_path: JSONL文件路径
    :param output_file: 输出文件路径
    :param max_lines: 最大处理行数，默认100行
    """
    # 读取JSONL数据（限制前100行）
    data_list = read_jsonl_data(jsonl_file_path, max_lines)
    if not data_list:
        print("无法读取JSONL文件内容，程序退出")
        return
    ...
    for idx, data in enumerate(data_list, 1):
        print(f"\n{'='*60}")
        print(f"处理第 {idx}/{len(data_list)} 条数据")
        print(f"源文件：{data['source']}")
        print(f"内容长度：{len(data['full_content'])} 字符")
        
        # 使用full_content作为完整语料
        novel_text = data['full_content']
        source_filename = data['source']
        
        # 运行 NovelHopQA 构造流程
        qa_pairs = novelhopqa_constructor(novel_text, source_filename)
        all_qa_pairs.extend(qa_pairs)
        
        print(f"第 {idx} 条数据处理完成，生成 {len(qa_pairs)} 个QA对")
    ...
    if all_qa_pairs:
        ...
        print(f"\n{'='*60}")
        print(f"所有数据处理完成！最终QA对已保存至：{output_file}")
        print(f"总QA数量：{len(all_qa_pairs)}")
    else:
        print("\n所有数据处理完成，但未生成有效QA对")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_file", "-i", type=str, required=True,
                       help="输入的文件路径（支持txt或jsonl格式）")
    parser.add_argument("--output_file", "-o", type=str,
                       default="novelhopqa_qa_pairs.jsonl",
                       help="输出的QA对文件路径（默认：novelhopqa_qa_pairs.jsonl）")
    parser.add_argument("--file_type", "-t", type=str, choices=["txt", "jsonl"],
                       help="文件类型（txt或jsonl），如果不指定则根据文件扩展名自动判断")
    parser.add_argument("--max_lines", "-n", type=int, default=100,
                       help="JSONL文件最大处理行数（默认：100行）")
    ...
    if args.file_type is None:
        if args.input_file.endswith('.jsonl'):
            ...
            print("错误：无法自动判断文件类型，请使用 --file_type 参数指定")
            exit(1)
    else:
    ...
    if file_type == 'jsonl':
        # 处理JSONL文件（限制前100行）
        print(f"将处理JSONL文件的前 {args.max_lines} 行")
        process_jsonl_file(args.input_file, args.output_file, args.max_lines)
    else:
        # 处理TXT文件（保持原有逻辑）
